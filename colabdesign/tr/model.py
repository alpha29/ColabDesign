...
from colabdesign.shared.utils import copy_dict, dict_to_str, update_dict
...
class mk_tr_model(design_model):
    def __init__(
        ...
        self._data_dir = (
            "."
            if os.path.isfile(os.path.join("models", "model_xaa.npy"))
            else data_dir
        )
    ...
    def prep_inputs(
        ...
        **kwargs,
        ...
        if self.protocol in ["hallucination", "partial"]:
            ...
            for n in range(1, 6):
                p = os.path.join(
                    self._data_dir, os.path.join("bkgr_models", f"bkgr0{n}.npy")
                )
                self._inputs["6D_bkg"].append(
                    self._bkg_model(get_model_params(p), key, self._len)
                )
